...
\begin{tikzpicture}[
	...
	\draw[path] (X1star) to node[above] {$\beta_{1}^{\ast}$} (Ystar);
	\draw[path] (Xjstar) to node {$\beta_{j}^{\ast}$} (Ystar);
	\draw[path] (Xpstar) to node {$\beta_{p}^{\ast}$} (Ystar);
	% error variance
	...
	\draw[cov] (varepsilonstar) to[out = 290, in = 250, looseness = 5] node[below] {$\sigma^{\ast 2} := 1 - \boldsymbol{\beta}^{\ast \prime}\mathbf{P}_{\mathbf{X}, \mathbf{X}}\boldsymbol{\beta}^{\ast}$} (varepsilonstar);
\end{tikzpicture}
